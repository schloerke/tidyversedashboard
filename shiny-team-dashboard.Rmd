...
repo_infos <-
  ...
  map_dfr(function(orgrepo) {
    data_frame(org = orgrepo[[1]], repo = orgrepo[[2]])
  })
...
---
title: "Shiny Team repo dashboard"
---
...
### Pull reqquests `r duration` change
```{r}
...
### Issues opened
```{r}
valueBox(issue_opened_sum, icon = "fa-exclamation-circle", color = "success")
```

### Issues closed
```{r}
valueBox(issue_closed_sum, icon = "fa-times-circle", color = "danger")
```
...
data_table(
  ...
  list(
    order = list(
      list(4, "desc")  # updated from oldest to newest
    )))
